...
export function useProblemFilters(filters) {
  ...
  return useMemo(() => {
    const { search = '', difficulty = '', topic = '', pattern = '', confidence = '' } = filters || {};
    const q = search.trim().toLowerCase();
    return problems.filter((p) => {
      if (q && !(p.title.toLowerCase().includes(q) || p.topic.toLowerCase().includes(q) || p.pattern.toLowerCase().includes(q))) return false;
